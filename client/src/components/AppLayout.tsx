...
import { LayoutDashboard, Settings, LogOut, Image, ImagePlus, FileText, Palette, RefreshCw, Video, BookText, Menu, X, BookOpen, PenTool, Users, BarChart3 } from "lucide-react";
...
const navItems = [
  ...
  // ANALYTICS (10)
  { icon: BarChart3, label: "Creative Performance", path: "/analytics" },
  // SYSTEM (11-14)
  ...
  return (
    <div className="min-h-screen bg-[#01040A] flex">
      ...
      `}>
        ...
        {/* Section: Analytics */}
        <div className="px-3 pt-5 pb-1">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-600 px-3">Analytics</p>
        </div>
        <nav className="px-3 space-y-0.5">
          {navItems.slice(10, 11).map((item) => {
            const isActive = location === item.path || location.startsWith("/analytics");
            return (
              <button
                key={item.path}
                onClick={() => handleNavClick(item.path)}
